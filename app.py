...
@st.cache_resource
def load_models():
    # Ambil jalur folder tempat file app.py ini berada secara absolut
...
if tfidf_model is None or knn_model is None:
    st.error("🚨 **Kritis:** File `tfidf_model (8).pkl` atau `knn_model (8).pkl` tidak terdeteksi di root direktori project.")
    st.warning("👉 Pastikan nama file di GitHub sesuai dengan nama file di kode (`tfidf_model (8).pkl` & `knn_model (8).pkl`).")
else:
    ...
    with tab2:
        ...
        if uploaded_file is not None:
            # Mengatasi error pemisah titik koma (;) atau koma (,) secara otomatis
            ...
            with c_prev:
                ...
                st.caption("Sistem akan otomatis memprediksi sentimen ulasan berdasarkan model KNN terpilih.")
                
            if btn_batch:
                with st.spinner("Sedang memproses visualisasi data..."):
                    # --- CLEANSING KETAT DATA INPUT ---
                    # Buang data null di kolom target, ubah ke string murni, dan buang spasi kosong
                    df_clean = df_batch.dropna(subset=[nama_kolom]).copy()
                    df_clean[nama_kolom] = df_clean[nama_kolom].astype(str).str.strip()
                    df_clean = df_clean[df_clean[nama_kolom] != ""].copy()
                    
                    total_data = len(df_clean)
                    
                    if total_data == 0:
                        st.error("🚨 **Error:** Tidak ditemukan teks ulasan yang valid pada kolom yang lu pilih. Coba cek kolom lain!")
                    else:
                        # Logika Jalur Klasifikasi: Utamakan Prediksi ML Baru Menggunakan Model KNN Lu
                        if 'Rating_Bintang' in df_clean.columns and pd.api.types.is_numeric_dtype(df_clean['Rating_Bintang']):
                            # Kondisi jika user mengunggah dataset yang memiliki rating bintang terstruktur bawaan
                            df_clean['Label'] = np.where(df_clean['Rating_Bintang'].isin([1, 2]), 0, 1)
                        else:
                            # Transformasi teks ulasan baru ke TF-IDF lalu prediksi menggunakan KNN
                            fitur_batch = tfidf_model.transform(df_clean[nama_kolom])
                        ...
                        with m1:
                            st.markdown(f'<div class="custom-card"><h5>📦 Total Data Valid</h5><h2>{total_data} <span style="font-size:14px;color:#6B7280;">Ulasan</span></h2></div>', unsafe_allow_html=True)
                        with m2:
                        ...
                        with g_table:
                            ...
                            # Saring ulasan organik secara aman dari error manipulasi string data baru
                            df_organik = df_clean[
